...
function submit(){{if(state.locked)return;const q=quiz.questions[state.index],answer=selected(q);if(!answer){{quizView.querySelector('[data-answer-input]')?.focus();return}}const result=grade(q,answer),correct=result.correct;state.answers.push({{id:q.id,answer,correct,approximate:result.approximate,similarity:result.similarity,knowledgePoint:q.knowledge_point,prompt:q.prompt}});state.locked=true;quizView.querySelectorAll('input,textarea').forEach(el=>el.disabled=true);const spelling=result.approximate?`<p><b>Spelling note · 拼写提示:</b> ${{Math.round(result.similarity*100)}}% similar; recommended spelling: ${{esc(result.expected)}}</p>`:'';quizView.querySelector('[data-feedback]').innerHTML=`<div class="erc-feedback" data-correct="${{correct}}"><strong>${{correct?(result.approximate?'Correct with a spelling note! 拼写近似，按正确计':'Correct! 答对了'):'Keep going! 再想一想'}}</strong>${{correct?'':`<p><b>Answer:</b> ${{esc(q.accepted_answers[0])}}</p>`}}${{spelling}}<p>${{esc(q.explanation_en)}}</p><p>${{esc(q.explanation_zh)}}</p></div>`;quizView.querySelector('.erc-actions').innerHTML=`<button type="button" class="erc-primary" data-action="next">${{state.index===quiz.questions.length-1?'查看成绩':'下一题'}}</button>`;quizView.querySelector('[data-action="next"]')?.focus()}}
