...
def validate_quiz(data: Any) -> dict[str, Any]:
    ...
    if not isinstance(questions, list) or len(questions) < 5:
        raise ValueError("questions must contain at least 5 items")
    ids: set[str] = set()
    types: set[str] = set()
...
<div class="erc-panel"><header class="erc-hero"><div class="erc-heading"><p class="erc-kicker">Daily English Review · v{CARD_VERSION}</p><h2>{html.escape(data['title'])}</h2><p class="erc-source">{html.escape(data['source_summary'])}</p></div><button type="button" class="erc-settings-button" data-action="settings" aria-label="复习卡片设置">设置</button></header><div class="erc-body"><div data-view="start" class="erc-start"><p>{question_count} questions · Configurable practice · Bilingual feedback</p><button type="button" class="erc-primary" data-action="start">开始今日复习</button></div><div data-view="settings" hidden></div><div data-view="quiz" aria-live="polite" hidden></div><div data-view="result" aria-live="polite" hidden></div></div></div>
