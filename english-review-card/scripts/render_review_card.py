...
def validate_quiz(data: Any) -> dict[str, Any]:
    ...
    if not isinstance(questions, list) or len(questions) != 5:
        raise ValueError("questions must contain exactly five items")
    ids: set[str] = set()
    types: set[str] = set()
    ...
    if "multiple_choice" not in types or len(types) < 2:
        raise ValueError("quiz needs a multiple-choice question and at least two question types")
    return data
...
<div class="erc-panel"><header class="erc-hero"><p class="erc-kicker">Daily English Review</p><h2>{html.escape(data['title'])}</h2><p class="erc-source">{html.escape(data['source_summary'])}</p></header><div class="erc-body"><div data-view="start" class="erc-start"><p>5 questions · Mixed practice · Bilingual feedback</p><button type="button" class="erc-primary" data-action="start">开始今日复习</button></div><div data-view="quiz" aria-live="polite" hidden></div><div data-view="result" aria-live="polite" hidden></div></div></div>
...
function renderQuestion(){{const q=quiz.questions[state.index];const input=q.type==='multiple_choice'?`<fieldset class="erc-options">${{q.options.map((o,i)=>`<label class="erc-option"><input type="radio" name="erc-answer" value="${{esc(o)}}"><span>${{String.fromCharCode(65+i)}}. ${{esc(o)}}</span></label>`).join('')}}</fieldset>`:q.type==='short_answer'?'<textarea class="erc-text" data-answer-input aria-label="Your answer" placeholder="Write your answer in English…"></textarea>':'<input class="erc-text" data-answer-input aria-label="Your answer" placeholder="Type your answer…">';quizView.innerHTML=`<div class="erc-progress-row"><span>Question ${{state.index+1}} of 5</span><span>${{state.answers.filter(a=>a.correct).length}} correct</span></div><div class="erc-progress" aria-hidden="true"><span style="width:${{state.index*20}}%"></span></div><div class="erc-type">${{labels[q.type]}}</div><h3 class="erc-prompt">${{esc(q.prompt)}}</h3>${{input}}<div data-feedback></div><div class="erc-actions"><button type="button" class="erc-primary" data-action="submit">提交答案</button></div>`;state.locked=false}}
function submit(){{if(state.locked)return;const q=quiz.questions[state.index],answer=selected(q);if(!answer){{quizView.querySelector('[data-answer-input]')?.focus();return}}const correct=q.accepted_answers.some(expected=>norm(expected)===norm(answer));state.answers.push({{id:q.id,answer,correct,knowledgePoint:q.knowledge_point,prompt:q.prompt}});state.locked=true;quizView.querySelectorAll('input,textarea').forEach(el=>el.disabled=true);quizView.querySelector('[data-feedback]').innerHTML=`<div class="erc-feedback" data-correct="${{correct}}"><strong>${{correct?'Correct! 答对了':'Keep going! 再想一想'}}</strong>${{correct?'':`<p><b>Answer:</b> ${{esc(q.accepted_answers[0])}}</p>`}}<p>${{esc(q.explanation_en)}}</p><p>${{esc(q.explanation_zh)}}</p></div>`;quizView.querySelector('.erc-actions').innerHTML=`<button type="button" class="erc-primary" data-action="next">${{state.index===4?'查看成绩':'下一题'}}</button>`}}
function renderResult(){{quizView.hidden=true;resultView.hidden=false;const correctCount=state.answers.filter(a=>a.correct).length,missed=state.answers.filter(a=>!a.correct),detail=quiz.questions.map((q,i)=>`<div class="erc-result-item"><strong>${{state.answers[i].correct?'✓':'○'}} ${{esc(q.knowledge_point)}}</strong><p>${{esc(q.explanation_en)}} / ${{esc(q.explanation_zh)}}</p></div>`).join('');resultView.innerHTML=`<div class="erc-result"><p class="erc-kicker">Review complete</p><div class="erc-score">${{correctCount}}/5</div><p>${{correctCount===5?'Excellent — 全部答对！':'Nice work — 错题会变成下一轮练习。'}}</p></div><div class="erc-result-list">${{detail}}</div><div class="erc-actions"><button type="button" class="erc-primary" data-action="followup">${{missed.length?'复练错题':'再来一组'}}</button></div><div class="erc-status" role="status"></div>`}}
async function followup(){{const missed=state.answers.filter(a=>!a.correct),prompt=missed.length?`使用 $english-review-card 再生成一轮不同的英语复习题。只围绕这些错题知识点：${{missed.map(a=>a.knowledgePoint).join('；')}}。依据仍限于当前学习对话，不要重复原题措辞。`:'使用 $english-review-card 根据当前学习对话再生成一组不同的五道英语复习题，不要重复原题措辞。',status=resultView.querySelector('.erc-status');if(!window.openai?.sendFollowUpMessage){{status.textContent='当前客户端不支持跟进消息，请在对话中输入“复练错题”。';return}}const button=resultView.querySelector('[data-action="followup"]');button.disabled=true;status.textContent='正在交给 Codex 生成下一轮…';try{{await window.openai.sendFollowUpMessage({{prompt,title:missed.length?'生成错题复练':'再生成一组'}});status.textContent='已发送。'}}catch(error){{button.disabled=false;status.textContent='发送失败，请稍后重试。'}}}}
root.addEventListener('click',event=>{{const action=event.target.closest('[data-action]')?.dataset.action;if(action==='start'){{startView.hidden=true;quizView.hidden=false;renderQuestion()}}if(action==='submit')submit();if(action==='next'){{state.index+=1;state.index<5?renderQuestion():renderResult()}}if(action==='followup')followup()}});root.addEventListener('keydown',event=>{{if(event.key==='Enter'&&!event.shiftKey&&event.target.matches('input[data-answer-input]')){{event.preventDefault();submit()}}}})}})();</script></section>'''
