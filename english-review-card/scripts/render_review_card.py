...
<div class="erc-panel"><header class="erc-hero"><div class="erc-heading"><p class="erc-kicker">Daily English Review · v{CARD_VERSION}</p><h2>{html.escape(data['title'])}</h2><p class="erc-source">{html.escape(data['source_summary'])}</p><p class="erc-metrics" data-metrics>等待题目数据…</p></div><button type="button" class="erc-settings-button" data-action="settings" aria-label="复习卡片设置" disabled hidden>设置</button></header><div class="erc-body"><div data-view="loading" class="erc-skeleton" role="status" aria-live="polite"><span class="erc-skeleton-line short"></span><span class="erc-skeleton-line medium"></span><span class="erc-skeleton-block"></span><span class="erc-skeleton-block"></span><span class="erc-skeleton-block"></span><p class="erc-skeleton-status">卡片正在加载，题目就绪后会自动显示。</p></div><div data-view="start" class="erc-start" hidden><p>{question_count} questions · Configurable practice · Bilingual feedback</p><button type="button" class="erc-primary" data-action="start">开始今日复习</button></div><div data-view="settings" hidden></div><div data-view="quiz" aria-live="polite" aria-busy="true" hidden></div><div data-view="result" aria-live="polite" hidden></div></div></div>
